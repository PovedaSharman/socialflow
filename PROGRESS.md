...
- Added a fixed 21-file release-host account/tenant runner. It refuses to start without three explicit test/disposable-database flags, requires a database name containing `test`, launches one non-shell Jest child with `--runInBand` and caps that child at a 1 GB heap.
...
- Verification for the release-host gate slice: targeted Prettier completed under a 128 MB heap cap; the runner's fail-closed guard was exercised under 64 MB and exited before Jest; `package.json` parsed under 64 MB; the 64 MB account/tenant and resource-safety audits passed with 22 and 17 invariants respectively; `git diff --check` passed. The current 21-file Jest/PostgreSQL gate and GitHub Actions workflow were not executed and are not claimed.
...
- Added a production-only, fail-closed social-provider availability boundary. A provider must be an exact entry in `SOCIAL_PROVIDER_ALLOWLIST`, have a configured callback origin and satisfy its adapter credential requirements before it appears in connection, plug, tool or rule discovery or passes connection-initiation allow checks.
- Kept non-production adapters available for development and preserved internal resolution of existing stored connections for recovery/migration. The simulated provider remains separately opt-in and impossible to enable in production.
- Documented the allowlist as the operator's explicit platform-approval attestation, added it blank to the environment and production Compose templates, and added a provider-by-provider sandbox release procedure without claiming approval or live credentials.
...
- Added the OAuth transaction, production provider-availability and safe publish-error-history unit suites to the guarded release-host account/tenant manifest. The gate remains one sequential Jest process with the existing 1 GB ceiling and disposable-database guards; its current fixed manifest contains 21 files.
- Verification for the expanded manifest: the runner's fail-closed guard was exercised under 64 MB and exited before Jest; the 64 MB account/tenant audit passed with 22 invariants. The 21-file Jest/PostgreSQL gate was not executed and is not claimed.
...
- Replaced full-payload publish error history with a bounded safe projection: redacted 1,000-character messages and at most 50 attempt summaries containing only post/integration/provider identifiers, state and root/comment kind. Current post errors use the same redaction, and debug history reads are capped at the newest 100 rows.
...
- Added Stripe webhook replay protection (Redis NX by event id) and a production test-mode gate that rejects `sk_live_` until `ALLOW_STRIPE_LIVE_MODE=true`. Documented in `docs/BILLING.md`.
