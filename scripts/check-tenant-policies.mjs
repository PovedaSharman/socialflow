...
export function findMissingTenantPolicies() {
  const missing = [];
  ...
  for (const file of protectedControllers) {
    ...
    for (const [index, line] of lines.entries()) {
      ...
      if (explicitlyNonMutatingOrSeparatelyAuthorised.has(`${file}:${route}`)) {
        continue;
      }
      ...
      if (!decoratorsAndSignature.includes('@CheckPolicies(')) {
        missing.push(`${file}:${index + 1}: ${route}`);
      }
    }
  }
...
const missing = findMissingTenantPolicies();
if (missing.length > 0) {
  process.stderr.write(`Missing tenant policies:\n${missing.join('\n')}\n`);
  process.exitCode = 1;
} else {
  process.stdout.write(
    `Tenant mutation policy audit passed for ${protectedControllers.length} controllers.\n`
  );
}
